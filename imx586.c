...
/*
 * Sony IMX586 CMOS sensor driver – NXP i.MX8/9 edition
 *
 * Generic V4L2 sub-device driver compatible with NXP CSI‑2 receiver blocks.
 */
...
#define IMX586_REG_MODE_SELECT 0x0100
...
static const struct regmap_config imx586_regmap_cfg = {
	.reg_bits = 16,
	.val_bits = 8,
	.max_register = 0xFFFF,
};
...
{
	if (priv->xclk) {
		int ret = clk_prepare_enable(priv->xclk);
		if (ret)
			return ret;
	}
	if (priv->reset_gpio) {
...
static void imx586_power_off(struct imx586 *priv)
{
	if (priv->reset_gpio)
...
static int imx586_set_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		if (ret)
			return ret;
		ret = regmap_write(priv->regmap, IMX586_REG_MODE_SELECT, MODE_STREAMING);
	} else {
		ret = regmap_write(priv->regmap, IMX586_REG_MODE_SELECT, MODE_STANDBY);
	}
	if (!ret)
		priv->streaming = on;
	return ret;
...
{
	struct imx586 *priv = container_of(sd, struct imx586, sd);
	return on ? imx586_power_on(priv) : (imx586_power_off(priv), 0);
}
		    ...
		    const struct i2c_device_id *id)
{
	...
	priv->regmap = devm_regmap_init_i2c(client, &imx586_regmap_cfg);
	if (IS_ERR(priv->regmap))
		return PTR_ERR(priv->regmap);

	priv->reset_gpio = devm_gpiod_get_optional(dev, "reset", GPIOD_OUT_LOW);
	priv->xclk = devm_clk_get_optional(dev, "xclk");
...
MODULE_DESCRIPTION("Sony IMX586 camera driver (i.MX8/9)");
